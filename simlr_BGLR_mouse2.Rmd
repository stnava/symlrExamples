...
bestVarx = 0
nEmbed = 3
mysq = c( 0.95, 0.5 ) # very sparse for demonstration purposes
mixAlg = 'svd' # this is what is used by RGCCA - so we do the same thing here
initu = initializeSimlr( inmats, nEmbed, jointReduction = T, uAlgorithm = mixAlg )
for ( k in c(99) ) { # take best result over multiple initial starting points
  temp = simlr(
    inmats,
    regs,
    iterations = 100,
