...
if ( ! exists( "mysym" ) | TRUE ) {
  mysym = symlr(
      ...
      mixAlg = 'svd', randomSeed = as.numeric(Sys.time()),
...
plot( template, iMath(abs(tImgs[[1]]),"Normalize")*1, window.overlay=c(0.1,1), axis=3, nslices=16 )
plot( template, iMath(abs(fImgs[[3]]),"Normalize")*1, window.overlay=c(0.1,1), axis=3, nslices=16 )
plot( template, iMath(abs(cImgs[[1]]),"Normalize")*1, window.overlay=c(0.1,1), axis=3, nslices=16 )
```
