...
initu = initializeSyMLR( A, 2, jointReduction=T, uAlgorithm = 'ica' )
if ( ! exists( "eType" ) ) eType = 'cca'
sResult = symlr( A, smoothingMatrices=regs,
  sparsenessQuantiles = c(0.0, 0.0, 0.0), # general purpose defaults
  positivities =  c('either','either','either'),
  initialUMatrix = initu, iterations = 50,
  lineSearchRange = c(-100,100),
  mixAlg = 'pca', orthogonalize = F,
  energyType = eType,
  scale = T,
  verbose = T )
