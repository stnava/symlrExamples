...
for ( sim in c(sim:nsims)  ) {
  ...
  nEmbeddings = nk = sample( 5:25, 1 )# for latent signal
    ...
    result = simlr(
      ...
      verbose = FALSE,
      iterations = nits,
      constraint = 'Stiefel', jointInitialization=FALSE,
...
*for the second hidden signal*
```{r}
# first signal
print(t.test(simdatafrm[,"symRSQ1"],simdatafrm[,"sgccaRSQ1"],paired=T))

# second signal
print(t.test(simdatafrm[,"symRSQ2"],simdatafrm[,"sgccaRSQ2"],paired=T))
```
...
mdata <- data.frame(
  method = rep(
    ...
    "SGCCA1", "SGCCA2" ),  each = nsims ),
  value = c( simdatafrm[,1], simdatafrm[,3], simdatafrm[,2], simdatafrm[,4]  )
)
