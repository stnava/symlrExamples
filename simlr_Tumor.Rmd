...
regs[[3]] = diag( 3 )
initu = initializeSimlr( Anew, 2, jointReduction=T, uAlgorithm = 'ica' )
sResult = simlr( Anew,
  smoothingMatrices = regs,
  initialUMatrix = initu,
  iterations = 50,
# make sure that we select separate columns for location by setting 0.9 for last term
# while we are at it, use fairly non-sparse selection for the first two matrices
  sparsenessQuantiles = c(0.1,0.1,0.9),
  energyType = energyType,
  mixAlg = 'ica',
  orthogonalize = TRUE, # illustrate effect of GS
  verbose = T )
