...
initu = # snpd$projections # init with cca
  initializeSyMLR( inmatsScl, nv, jointReduction = F, uAlgorithm = 'ica' )
# vinit = list(
#  t(t(initu[[1]]) %*% inmatsScl[[1]]),
#  t(t(initu[[2]]) %*% inmatsScl[[2]])
# )
if ( ! exists( "eType" ) ) eType = 'cca'
if ( ! exists( "doOrth" ) ) doOrth = FALSE
...
sResult = symlr(
  inmats,
      ...
      positivities =  c('positive','positive'), # general purpose defaults
      initialUMatrix = initu,
      vmats = vinit,
      verbose = 1,
      scale = 'np',
      orthogonalize = doOrth,
